...
import { verifyToken, UserToken } from './lib/auth';
...
export async function proxy(request: NextRequest) {
  ...
  const user = token ? (verifyToken(token) as UserToken) : null;
  ...
  if (pathname.startsWith('/seller/')) {
    ...
    const seller = await pool.query('SELECT id FROM sellers WHERE user_id=$1', [user.id]);
